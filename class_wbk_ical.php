    ...
    static public function generateICal( $appointment_ids, $type = 'admin' ) {
        if ( wbk_fs()->is__premium_only() ) {
            if ( wbk_fs()->can_use_premium_code() ) {
                ...
                $a = new \ReflectionClass('\Eluceo\iCal\Component\Calendar');

                error_log( 'Class path: ' . $a->getFileName());
                ...
                foreach($appointment_ids as $appointment_id) {
                    ...
                    error_log( 'Appointment getTime(): ' . $appointment->getTime());
                    ...
                    if( $range_selection == 'disabled' ){
                        error_log( 'Renage selection disabled'  );
                        ...
                        $vCalendar->addComponent($vEvent);
                         ...
                         error_log( 'Renage selection enabled'  );
                        if( $initial_start == '' ){
                            $initial_start = $start;
                        }
                    }
                ...
                $filename =    get_temp_dir() . 'calendar_' . $file_prefix . implode( '_', array_values($appointment_ids) ) . '.ics';
